...
const CheckList = () => {
  ...
  const updateRequestLoad = () => {
    ...
    const requestedLoad = loadsList.filter((load) => load.requestNumber === requestNumber)[0] as
      | Data
      | any;
  ...
  function handleSaveState() {
    ...
    checkIfAllVolumesAreSucessful(requestedLoad.volumes)
      ? (requestedLoad.status = "sucess")
      : (requestedLoad.status = "fail");

    const newLoadsList = () =>
      loadsList.map((load) =>
        load.requestNumber === requestNumber ? load.volumes : load
      ) as Array<Load>;

    setLoadsList(newLoadsList);

    storeData("@loadsList", loadsList).then(() => navigation.goBack());
  }
  ...
  if (hasPermission === false) {
    <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
      <LoadingAnimation />
      <Text style={{ color: "#E53035", fontSize: 24, fontWeight: "bold", marginTop: 10 }}>
        ERRO SEM ACESSO A CAMERA
      </Text>
    </View>;
  }
